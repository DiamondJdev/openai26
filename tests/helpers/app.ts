...
export interface AppHarness { readonly ctx: AppContext; readonly tmp: string; cleanup(): Promise<void>; }

async function writePng(file: string, background: string): Promise<void> {
  await sharp({ create: { width: 320, height: 240, channels: 3, background } }).png().toFile(file);
}

/** Build a self-contained AppContext with seeded still-image footage for service tests. */
export async function buildAppHarness(plate = "TEST-123"): Promise<AppHarness> {
  ...
  const ctx: AppContext = {
    ...
    manifestLoaded: true,
    env: { openAiApiKey: "", model: "gpt-5.6", publicBaseUrl: "http://localhost:3000", databaseUrl: "", manifestPath: "", maxInvestigationMs: 45_000 },
  };
  return {
    ctx,
    tmp,
...
export async function jpegBuffer(): Promise<Buffer> {
  return sharp({ create: { width: 200, height: 150, channels: 3, background: "#999" } }).jpeg().toBuffer();
}
