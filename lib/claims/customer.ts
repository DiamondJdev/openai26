 ...
 */
export async function verifyAndStartSession(
  ctx: AppContext,
  token: string,
  pin: string,
  nowMs: number = Date.now(),
): Promise<SessionResult> {
  const access = await getAccessByTokenHash(ctx.db, hashToken(token));
...
export async function submitIntake(
  ...
  for (const { kind, image } of stored) {
    const storedPath = `claimlens/uploads/${claimId}/${newId("upload")}.jpg`;
    await ctx.artifacts.putJpeg(storedPath, image.data);
  ...
  await updateClaimStatus(ctx.db, claimId, "customer_submitted");
}
...
export async function getCustomerView(
  ...
  const report = await getReportByClaimId(ctx.db, claimId);
  if (!report) return { state: "under_review" };

  const crops = claim.shareEvidenceCrops
    ? (await listCropsByClaim(ctx.db, claimId)).map((c) => ({
        id: c.id,
        region: c.region,
        camera: c.camera,
      }))
...
export async function claimHasAccess(ctx: AppContext, claimId: string): Promise<boolean> {
  return (await getAccessByClaimId(ctx.db, claimId)) !== null;
}
